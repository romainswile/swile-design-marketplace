...
1. **Un seul serveur** : `figma_get_status` → **`portFallbackUsed` DOIT être `false`** (et `otherInstances` vide s'il est présent — champ absent des sorties récentes : ne conclus jamais « pas d'orphelin » de son absence). `portFallbackUsed:true` = un autre serveur tient le port préféré → inventaire (`lsof -i :9223-9232` / `netstat -ano`) et kill des orphelins, PAS un caveat à noter (mécanisme : plusieurs serveurs se disputent le websocket du plugin → déconnexions spontanées dès les premières minutes). ⚠️ Tuer l'orphelin ne suffit PAS à revenir à `false` : un serveur ne migre jamais de port — enchaîne avec le **RESTART AUTONOME §3.2-a** (kill du serveur actif → respawn sur 9223). Demande à l'user (question à choix, **1ʳᵉ option : « Je les tue automatiquement (Recommandé) »**) puis — Windows (PowerShell) : `taskkill /PID <pid> /F` · macOS/Linux : `kill -9 <pid>`. ⚠️ **Tuer les orphelins AVANT toute réouverture du plugin** (scan des ports → rattachement à un orphelin).
...
**Balayage final des résidus (AVANT le rapport)** : scanne la page — tout nœud de PREMIER niveau qui n'est ni une section source, ni un root repro (`state.roots`), ni un élément attendu = résidu de session (KIT, scratchs, textes/clones d'essai orphelins). Poste la liste `{id, nom, type, x, y}` → supprime-les → re-scan collé (page propre). Un résidu À L'INTÉRIEUR d'un écran est du ressort de verify/compare, pas de ce balayage.
...
2. **Canal empoisonné** — déclencheur : **2 gels d'import consécutifs**, confirmés par l'**IMPORT-TEST** (⚠️ le probe ne détecte PAS un gel — les lectures passent toujours ; le seul détecteur fiable est un import chronométré d'une clé déjà cachée : ~15 ms = sain, timeout = gelé). N'insiste pas en contournements : ils coûtent plus cher qu'un restart.
   **2-a. RESTART AUTONOME du serveur (0 action user — testé bout-en-bout)** : re-§0.1 d'abord (orphelins) ; PID du serveur = le listener du port affiché par `get_status` (`netstat -ano` / `lsof -i :<port>`) ; tue-le (Windows PowerShell : `taskkill /PID <pid> /F` · macOS : `kill -9 <pid>`) ; **pause 5 s** ; appelle `figma_get_status` — c'est CET appel qui déclenche le respawn (le serveur re-bind 9223 → corrige aussi `portFallbackUsed`) ; le plugin Desktop Bridge se rattache SEUL : **poll `get_status` avec pauses de 10 s, 3 essais max**, jusqu'à `setup.valid:true` + les 3 fichiers connectés. Effets prouvés : `code.js` rechargé (sandbox purgé → §3.5 obligatoire ensuite), port corrigé, bouchons du worker purgés. Termine par un IMPORT-TEST.
   **2-b. Import-test encore gelé après 2-a** (gel profond côté Figma — mesuré : le restart serveur ne le guérit pas) OU le plugin ne se rattache pas en 3 essais (Figma fermé ?) → demande à l'user : « ferme et rouvre le plugin Desktop Bridge (3 fichiers), puis dis-moi ok ». Ensuite : re-§0 → **§3.5** → **warm-up immédiat** (les clés gelées repassent en ~0,3 s sur canal frais ; le canal se re-dégrade en ~10 min et supporte peu d'imports une fois tiède). La réouverture ne remplace PAS §3.7 : styles durablement introuvables → §3.7 reste l'issue.
