...
# swile-design v3.8.0 — écrans Figma → DS « 🏢 Flõw | Corporate » (shadcn)
...
```
**Le pourquoi vit dans les registres, jamais seulement dans la prose.** Toute décision non triviale (compromis, tie-break de sonde serré, choix d'accent, skip) porte un champ `pourquoi` sur son entrée MAPPING/LEDGER — le raisonnement, pas seulement le choix. Raison mécanique : les artefacts lourds (dumps, verify/compare/sonde) sont **jetables après réconciliation** et disparaissent à la moindre compaction du contexte ; ce qui doit survivre — le *pourquoi* de chaque choix — n'est garanti que s'il est **dans le state persisté** (`.swile-state.json`, §2.6), pas dans l'historique de conversation. ⚠️ **La compaction résume la conversation, pas les règles du skill** : rien ne garantit qu'un résumé automatique conserve la discipline fine (1 import max/call, formats des registres, etc.) avec la même fidélité qu'une lecture fraîche — si un comportement dérive juste après une compaction (import groupé, registre mal formé), **relis ce fichier avant de continuer** plutôt que de présumer que le résumé a tout gardé.
...
// tie-break de sonde serré (2 candidats proches) → ajoute pourquoi sur l'entrée à la mise à jour SONDE→DS : ...,pourquoi:'Outline retenu : stroke #e5e5e5 visible sur la source, dist fond 4 vs Ghost sans stroke'
```
...
- **Test skip/compromis, appliqué AU MOMENT du choix** : élément ou propriété **visible** de la source absent de la repro = **SKIP, quel que soit le mot que tu emploies** → GATE (stop, préviens, demande, attends). Composant DS présent mais valeur divergente (token/taille au plus proche) = **compromis** → `LEDGER.push({ecran, element, type:'COMPROMIS', source:'…', choix:'…', pourEgaler:'…', pourquoi:'…'})` **immédiatement** (pas au rapport) — `pourquoi` = le raisonnement qui a mené au compromis (pas une paraphrase de `choix`), pour qu'une compaction ne l'efface pas. Un compromis dont `pourEgaler` se résout par un **simple import** (ex. « importer Ghost Icon Button ») = **refusé** : fais l'import isolé (§2.6), « coûteux » n'est pas un motif.
...
**Checkpoint après chaque écran vérifié = POINT DE COMPACTION SÛR** : `figma.saveVersionHistoryAsync('<écran> vérifié')` **+ mets à jour sentinelle et état persisté** (outil Write) :
...
**Une fois ce flush écrit et `reconcile().ok` obtenu, l'écran est intégralement sauvegardé sur disque** : ses artefacts lourds (dump source, sorties de verify/compareToSource/textDiff/sonde de CET écran) ne servent plus à rien et sont **jetables**. C'est donc le seul endroit où réduire le contexte est sans risque : une **compaction ici — automatique (Claude Code) ou manuelle (`/compact`, décidée par l'user, jamais par toi) — est sans perte pour les REGISTRES**, car §3.5 réhydrate registres + clés + reconcile depuis le state (cache quasi instantané). ⚠️ **Mais la compaction résume, elle ne relit pas ce fichier** : si ton comportement dérive après coup (import groupé, oubli d'un champ `pourEgaler`/`pourquoi`, format de registre approximatif), c'est le signe que la discipline fine s'est diluée dans le résumé — **relis ce SKILL.md avant de continuer**, ne présume pas qu'elle a survécu telle quelle. N'attends pas la compaction et ne la force pas : n'interromps pas ton tour et ne touche pas à la sentinelle (elle reste `en_cours`/`clean:false` tant qu'il reste des écrans — sinon le hook te bloquerait). Poste simplement, en une ligne, `✅ <écran> réconcilié — checkpoint écrit, compaction sûre à partir d'ici`, puis enchaîne l'écran suivant. Ne re-cite jamais les dumps d'un écran déjà réconcilié : le state fait foi.
