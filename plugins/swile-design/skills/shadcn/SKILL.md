...
**WARM-UP (testé)** — construis `COMP_KEYS`/`VAR_KEYS`/`STYLE_KEYS` (`[['nom','clé'],…]`) depuis mapping+sonde+annexe **pour TOUS les écrans demandés** (composants, icônes, tokens dimension, text styles, templates SHADCN) — un import mi-série est un mode dégradé (§3), pas le plan. **Minimise la liste par le harvest** : si un template SHADCN va être posé, importe SA clé en premier, pose l'instance, inventorie ses styles/variables vivants (§ échelle ci-dessus) — et retire des KEYS tout ce que cet inventaire couvre : chaque import évité est un gel évité. Puis (call `timeout:30000`) :
```js
