...
# swile-design v3.6.4 — écrans Figma → DS « 🏢 Flõw | Corporate » (shadcn)
...
0. **Skill à jour ? — GATE BLOQUANT : ce skill ne tourne JAMAIS sur une version périmée.** `curl -s https://raw.githubusercontent.com/romainswile/swile-design-marketplace/master/.claude-plugin/marketplace.json` → compare `version` à celle du titre de ce document. Plus récente → exécute directement (sans question) `claude plugin marketplace update swile-marketplace && claude plugin update swile-design@swile-marketplace`, écris la sentinelle `{"etat":"bloque","motif":"mise à jour vX.Y.Z installée, relance requise"}`, puis **STOP NET** avec EXACTEMENT ce message (la version chargée ne change pas en cours de session — « ok » seul relancerait l'ancienne) :
> ⬆️ **Plugin mis à jour vA.B.C → vX.Y.Z — je m'arrête ici** (interdiction de tourner sur une version périmée). Pour reprendre sur la nouvelle version : ouvre une **nouvelle session** et relance ta commande `/swile-design:shadcn …` (raccourci sans changer de session, selon ta version de Claude : `/reload-skills` ou `/reload-plugins`, puis relance la commande). Rien d'autre à faire.
Anti-fausse-alerte : si `claude plugin list` montre l'installée = distante, la mise à jour est déjà faite — même STOP, même message (sans relancer l'update). **Auto-guérison (avant le STOP)** : si `~/.claude/settings.json` → `extraKnownMarketplaces.swile-marketplace` existe sans `"autoUpdate": true`, ajoute la clé (préserve le reste du fichier) — les prochains démarrages seront à jour nativement et ce gate ne se déclenchera plus. `claude` introuvable (rare) → même STOP, avec `/plugin marketplace update swile-marketplace` puis `/plugin` → Installed → update dans le message. Pas de réseau/curl (invérifiable) → continue, mais note LEDGER `{type:'VERSION-NON-VERIFIEE'}` et signale-le dans le rapport.
...
globalThis.readNode = async (id) => { const n=await figma.getNodeByIdAsync(id); if(!n) return {missing:true,id};
  ...
  if('findAll'in n){ const vec=n.findAll(x=>x.type==='VECTOR'&&x.visible!==false&&!insideCloneCS(x)&&((Array.isArray(x.fills)&&x.fills.some(p=>p.type==='SOLID'&&p.visible!==false))||(Array.isArray(x.strokes)&&x.strokes.length)))[0];
    ...
    r.texts=n.findAll(x=>x.type==='TEXT'&&x.visible!==false&&!insideCloneCS(x)).map(t=>t.characters).join('|').slice(0,120); }   // clone exclu (ses textes sont couverts par textDiff)
  return r; };
