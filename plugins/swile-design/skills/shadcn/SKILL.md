...
4. **DS abonnée** (cible = fichier de travail) : via `figma_execute` — `await figma.teamLibrary.getAvailableLibraryVariableCollectionsAsync()` liste des collections « Corporate ». Sinon STOP → Assets > Libraries. *(PAS `figma_get_library_variables`.)*
