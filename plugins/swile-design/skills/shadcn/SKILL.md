...
# swile-design v3.12.0 — écrans Figma → DS « 🏢 Flõw | Corporate » (shadcn)
...
```js
const customs = new Set(MAPPING.filter(m=>m.ecran===globalThis.ECRAN && m.statut==='custom').map(m=>m.choix)).size;
const compromis = new Set(LEDGER.filter(l=>l.ecran===globalThis.ECRAN && l.type==='COMPROMIS').map(l=>l.choix)).size;
```
