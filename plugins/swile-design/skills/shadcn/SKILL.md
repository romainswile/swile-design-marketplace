...
# swile-design v3.13.0 — écrans Figma → DS « 🏢 Flõw | Corporate » (shadcn)
...
- **update/create** : saute **uniquement le relevé de la source legacy** de §2.1 — les règles outillées qui y vivent (dumpSource obligatoire pour TOUT relevé d'écran, budget captures) restent en vigueur. Mapping §2.2 obligatoire, réduit au **delta**, depuis la spec/maquette ou les écrans existants (lis-les). Tous les gates de §2.2 valent. **Cohérence (shell, layout, espacements, tailles, variantes)** : le scan Templates (§2.2-0) reste **TOUJOURS dû**, qu'une référence user soit donnée ou non — il ne se substitue jamais, il s'additionne. Si l'user désigne **en plus** une référence explicite dans son message de lancement (un écran shadcn déjà construit) → suis-la pour tout ce que le template ne dicte pas explicitement. **Contradiction réelle entre la référence et le template** (même type d'élément, composants/tokens différents) → ne tranche jamais seul, GATE (signale les deux, demande). **Témoin = l'écran le plus complexe parmi ceux à modifier/créer (comme en convert : le plus de types couverts), traité EN ENTIER** — toutes ses modifications demandées, pas la première trouvée : jamais de STOP après un seul élément simple (ex. un bouton ajouté) en laissant le reste de cet écran sans vérification avant d'enchaîner. Passe compare : source = spec/maquette ; update sans spec = read-back avant/après ; **create sans maquette** = paires en `expect{}` + read-back rapproché de la spec textuelle. `verify`/`textDiff` restreints au sous-arbre modifié (§2.5) ; violations préexistantes ailleurs → signale sans corriger.
...
0. **Skill à jour ? — GATE BLOQUANT : ce skill ne tourne JAMAIS sur une version périmée.**
   ...
   **0-b. Check distant** : `curl -s https://raw.githubusercontent.com/romainswile/swile-design-marketplace/master/.claude-plugin/marketplace.json` → compare `version` à celle du titre. Plus récente → exécute directement (sans question) `claude plugin marketplace update swile-marketplace && claude plugin update swile-design@swile-marketplace`, **puis purge le cache périmé** (bug CLI connu — le cache est keyé par version exacte et sert l'ancienne copie sans vérifier les fichiers source, cf. issues #14061/#15621/#17361/#28492 ; symptôme observé : « update fait » suivi d'un reload qui recharge quand même l'ancienne version) : supprime tous les sous-dossiers de `~/.claude/plugins/cache/swile-marketplace/swile-design/` **sauf** celui de la version qui vient d'être installée (chaque dossier restant est un cache mort — rien ne le référence une fois `plugin.json` sur la nouvelle version), et pour `~/.claude/plugins/marketplaces/temp_*` supprime uniquement ceux dont `git -C <dossier> remote get-url origin` correspond à `romainswile/swile-design-marketplace` (ne touche jamais un `temp_*` d'un autre remote — staging partagé entre marketplaces). Puis écris `.swile-update.lock` = `{"version":"<nouvelle>"}` ET la sentinelle `.swile-verify.json` = `{"etat":"bloque","motif":"mise à jour installée, nouvelle session requise"}` (fichiers à la racine du répertoire de travail — définis en fin de §0), puis **STOP NET** avec EXACTEMENT ce bandeau (« ok » ne suffit pas : la version chargée ne change pas en cours de session, et le hook bloque toute reprise) :
...
**Clone-tel-quel d'une zone** (seulement si l'user le demande explicitement dans son message de lancement, ex. « sans menu de gauche, à cloner depuis la source » — peu importe la zone, pas seulement une sidebar ; sans demande explicite, la zone suit la reconstruction normale comme le reste de l'écran) : `node.clone()`, contenu inclus, sans reconstruction interne ; nomme le clone `<nom-zone> (cloné)` — `verify`/`compareToSource` exemptent automatiquement tout nœud dont un ancêtre finit par `(cloné)` (mécanisme : du contenu preservé tel quel depuis la source n'a pas à être conforme aux tokens DS, c'est justement ce que l'user a demandé).
...
- **Rien ne correspond** (aucun élément, aucun écran équivalent) → mapping classique ci-dessous. Ça n'annule pas pour autant l'intérêt de la page Templates : si un template existant partage un shell/layout/espacement/gabarit de variantes pertinent même sans élément identique, inspire-toi-en pour la cohérence inter-écrans (LEDGER `{type:'INSPIRE-TEMPLATE', tpl}` si utilisé) — la case "rien ne correspond" ne veut dire "aucune inspiration à en tirer" que si la page ne contient réellement rien d'apparenté au domaine de l'écran.
...
**Par écran, deux compteurs DÉDOUBLONNÉS** (aucun champ nouveau — dédoublonne par `choix`, pas par ligne : un même élément custom dupliqué sur l'écran — ex. une card répétée 2 fois avec la même solution — compte pour UN seul, la 2e occurrence est un clone de la même décision, pas une décision de plus). **`globalThis.ECRAN` n'est PAS utilisable ici** — il n'est posé qu'au §2.4 (build), qui tourne APRÈS ce point ; dérive la liste des écrans directement de MAPPING, déjà complet à ce stade :
```js
for (const ec of [...new Set(MAPPING.map(m=>m.ecran))]) {
  const customs = new Set(MAPPING.filter(m=>m.ecran===ec && m.statut==='custom').map(m=>m.choix)).size;
  const compromis = new Set(LEDGER.filter(l=>l.ecran===ec && l.type==='COMPROMIS').map(l=>l.choix)).size;
  // évalue le seuil ci-dessous pour CET écran (ec) avant de passer au suivant
}
```
...
**WARM-UP (testé)** — construis `COMP_KEYS`/`VAR_KEYS`/`STYLE_KEYS` (`[['nom','clé'],…]`) depuis mapping+sonde+annexe **pour TOUS les écrans demandés** (composants, icônes, tokens dimension, text styles, templates SHADCN) — un import mi-série est un mode dégradé (§3), pas le plan. **Ordre qui minimise les imports** (chaque import évité est un gel évité — les styles/variables sont les plus fragiles) : ① importe d'abord les **COMPOSANTS** (templates SHADCN, atomes, icônes — eux ne se harvestent pas) ; ② **inventorie** les styles et variables vivants qu'ils portent (masters importés + instances posées : `textStyleId`, `boundVariables.*` — § échelle ci-dessus) ; ③ n'importe par clé QUE les styles/variables que l'inventaire ne couvre pas (souvent presque rien). Concrètement : **DEUX passes détachées** (passe A = composants, pose du KIT + inventaire ; passe B = reliquat styles/vars). **La boucle détachée + fenêtre morte 120s ne se justifie QUE si le total de clés à importer (composants + variables + styles confondus, passe A et B ensemble) dépasse 3** — c'est le rituel lourd pensé pour un warm-up chargé (convert multi-écrans, 40+ clés). **Si le total est ≤3** (mesuré : un `create` minimal peut n'avoir besoin que d'1-2 clés), des calls dédiés `withTimeout` 20 s suffisent pour CHAQUE clé, passe A **et** B, sans boucle ni fenêtre — même un import de template SHADCN (~500 nœuds) isolé passe très bien par un call dédié quand c'est la seule clé du lot. Script d'une passe (call `timeout:30000`) :
```js
...
globalThis.verify = async (rootId) => {
  ...
  const insideClone=n=>{let p=n;while(p&&p.type!=='PAGE'){if(/\(cloné\)$/.test(p.name||''))return true;p=p.parent;}return false;};
  ...
  for(const n of [root,...root.findAll(()=>true)]){
    if(insideInst(n)||insideClone(n)) continue;   // intérieur d'instance + clone : hors scan (le clone est couvert par textDiff, §2.5)
...
```js
globalThis.insideCloneCS = n=>{let p=n;while(p&&p.type!=='PAGE'){if(/\(cloné\)$/.test(p.name||''))return true;p=p.parent;}return false;};
...
## A. ANNEXE — `references/annexe.md`
**Lis ce fichier dès le §0** (l'import-test de setup §0.6 utilise déjà une clé de l'annexe, avant même le mapping) — c'est un **cache de démarrage** (données mesurées lors des runs précédents), **PAS l'inventaire du DS** : la sonde re-confirme ce qui est utilisé, **absence dans l'annexe ≠ absence dans le DS/Library** → preuve de recherche scriptée obligatoire avant tout arrondi/substitution, une clé qui échoue se re-découvre par nom. Contient : correspondances mesurées (masters), page Templates, accent violet, pièges DS, variables theme, dimensions Tailwind, limites de l'exécuteur, text styles, clés de composants/icônes.
