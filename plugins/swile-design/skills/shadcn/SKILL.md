...
# swile-design v3.6.3 — écrans Figma → DS « 🏢 Flõw | Corporate » (shadcn)
...
> ⬆️ **Plugin mis à jour vA.B.C → vX.Y.Z — je m'arrête ici** (interdiction de tourner sur une version périmée). Pour reprendre sur la nouvelle version : tape **`/reload-plugins`** puis relance ta commande `/swile-design:shadcn …` — ou ouvre une **nouvelle session**. Rien d'autre à faire.
